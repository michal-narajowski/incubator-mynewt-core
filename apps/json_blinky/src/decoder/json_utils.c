...
int jbuf_write(void *buf, char* data, int len) {
    int i;
    for(i = 0; i < len; i++) {
...
char jbuf_read_next(struct json_buffer *jb) {
    char c;
    struct jbuf  *ptjb = (struct jbuf*) jb;
...
void buf_init(struct jbuf *ptjb, char *string) {
    ...
    ptjb->json_buf.jb_read_next = jbuf_read_next;
    ptjb->json_buf.jb_read_prev = jbuf_read_prev;
    ptjb->json_buf.jb_readn = jbuf_readn;
